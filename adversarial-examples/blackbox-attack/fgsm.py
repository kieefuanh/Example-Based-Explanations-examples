...
artifacts_folder = '../../artifacts/adversarial-examples/blackbox-attack/'
...
# calls model (STEP 2)
    ...
    with tf.variable_scope('pred'):
        ambiente.pred = tf.argmax(ambiente.ybar, axis=1)
...
# read images from dataset
    mnist = mnist_data.read_data_sets(artifacts_folder + 'data', one_hot=True, reshape=False, validation_size=0) 
...
# STEP 4
    training(sess, ambiente, X_train, y_train, X_validate, y_validate, shuffle=False, batch=128, epochs=3)
    evaluate(sess, ambiente, X_test, y_test)
    loss, acc = sess.run([ambiente.loss, ambiente.acc], feed_dict={ambiente.x: [X_test[0]], ambiente.y: [y_test[0]]})
    print('Test acc: ', acc)
    pred = sess.run(ambiente.pred, feed_dict={ambiente.x: [X_test[0]]})
    print('Prediction for test image 0: ', pred)
    cv2.imwrite(artifacts_folder + 'x_test0_'+ str(pred[0]) +'.jpg', X_test[0]*255)

    X_adv = perform_fgsm(sess, ambiente, X_test, eps=0.02, epochs=12)
    evaluate(sess, ambiente, X_adv, y_test)
    loss, acc = sess.run([ambiente.loss, ambiente.acc], feed_dict={ambiente.x: [X_adv[0]], ambiente.y: [y_train[0]]})
    print('Adv acc: ', acc)
    pred = sess.run(ambiente.pred, feed_dict={ambiente.x: [X_adv[0]]})
    print('Prediction for adversarial image 0: ', pred)
    cv2.imwrite(artifacts_folder + 'Adv_0_'+ str(pred[0]) +'.jpg', X_adv[0]*255)
